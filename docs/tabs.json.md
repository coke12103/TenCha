...
| `disable_global_filter` | 任意。真偽値。<br>全体に適用されるNG機能をこのタブに適用するか。 |
| `limit`                 | 任意。数値。<br>このタイムラインの投稿蓄積上限。指定されなれば200件。 |
| `skin_name`             | 任意。文字列。<br>このタイムラインの見た目。下記の組み込みのテーマの名前を指定します。|
...
|   ----   | ---- |
| `default`| デフォルトのテーマ。 |
| `sobacha`| TwitterクライアントのSobaChaの1行表示風のテーマ。|
